...
with open('tweet_data.csv', 'w') as file:
	...
	keywords = "tesla OR TSLA OR Elon Musk OR Electric Vehicle OR Tesla"
	...
	for tweet in tweepy.Cursor(api.search_full_archive, environment_name="research", query = keywords, fromDate = date_since, toDate = date_to).items(100):
		if tweet.retweeted:
			continue
		else:
			row = [tweet.created_at, tweet.user.name, tweet.user.name, tweet.user.id, tweet.text, tweet.user.statuses_count, tweet.retweet_count, tweet.favorite_count, tweet.user.followers_count, tweet.lang]
			print(row)
